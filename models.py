...
class ImagePost(db.Model):
	...
	def __init__(self,name,description,location,user):
		self.name = name
...
class PostHandler():
	...
	def get_random_image(self):
		lim = ImagePost.query.count()
		return ImagePost.query.get(int(lim*random())+1)
	...
	def get_next_url(self):
		url = Backends.query.get(1).url
		return url

	def post_image(self,name,desc,loc,user,tags):
		IP = ImagePost(name,desc,loc,user)
		db.session.add(IP)
		taglist = tags.split(' ')
		for tag in taglist:
